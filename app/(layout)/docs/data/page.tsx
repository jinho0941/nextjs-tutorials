...
const Page = () => {
  return (
    <ContentSection>
      ...
      <p>
        둘의 가장 큰 차이점은 memory는 프로그램이 실행중일때 존제하는
        데이터이며, 프로그램이 종료되거나 전원 공급이 끊어지면 사라지는 특징을
        가지고 있고,
        <br />
        storage는 프로그램의 실행유무와 상관없이 한번 저장이 되면 삭제하지 않는
        한 영구적으로 데이터가 존재하며,
        <br />
        댓글, 영상, 채팅 등 대부분 데이터가 저장이 되고 불러와야하는 특징이
        있어서, 저장되어야 하는 데이터라면 storage에 저장된다고 이해하시면
        됩니다.
      </p>
      <p>
